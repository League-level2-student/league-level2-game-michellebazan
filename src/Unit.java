...
//variables
	...
	// constructor
	Unit(int HP, int Atk, int Spd, int Def, int Res, int Mag, int Lck, Affinity element, int x, int y, int width,
			int height, String Name) {
		// The super keyword refers to superclass (parent) objects. It is used to call
		// superclass methods,
		// and to access the superclass constructor. The most common use of the super
	...
	void heal(Unit unit, boolean isMagic) {
		...
		while (numHeals > 0) {
			...
			if (isMagic == true) { // 100 success rate but num of hp is random
				// need to fix the equation for healing self if the unit has magic, check is it
				// needs to be unit.HP (etc.)
				int randnumm = randd.nextInt(HP); // figure out if you want to limit the heal option or not // can
													// change it so at least half of the total hp is return through heal
				...
				JOptionPane.showMessageDialog(null,
						unit.name + " health has increased by " + randnumm + " putting them at a total of " + HP);
			}
	...
	void attack(Unit unit, boolean isMagic) {
		...
		for (int i = 0; i < numAttacks; i++) {
			...
			// if we get a number that's under true hit rate --> we can deal damage to the
			// target unit using attack against their defense
